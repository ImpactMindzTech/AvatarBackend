...
import { stripeAccountSuccessEmail,refundSuccessEmail, accountConnectedEmail, accountupdateConnectedEmail, stripeAccountAddedAdminNotification, avatarWithdrawNotification } from "../../services/CreateEmail.js";
...
export const checkout = async (req, res) => {
  try {
    const { productId, product, avatarId, reqid,bookingId, price, Adminfee, paymentType } = req.body;
...
export const Paypalcheckout = async (req, res) => {
  ...
  try {
    const { productId, product, reqid, avatarId, bookingId, price, Adminfee, paymentType } = req.body;
    const { _id } = req.user;
...
export const paymentsuccess = async (req, res) => {
 ...
 const captureId = payment.transactions[0].related_resources[0].sale.id;
      ...
      if (error) {
        ...
        let updateaccount = await Payment.findOne({ paymentIntentId: paymentId });
        if (updateaccount) {
        ...
        let updatebooking   = await Booking.findOne({PaymentintendId:paymentId});
        if(updatebooking){
...
let username  = user.userName;
  ...
  try {
    ...
    if (stripeAccount) {
      ...
      sendEmail(email,"Stripe Account Update Successfully",accountupdateConnectedEmail(username,email));
      ...
      capabilities:capabilities,
        ...
        tos_acceptance: {
          service_agreement: ["GI", "US"].includes(countryCode) ? "full" : "recipient",// Fixes the ternary condition
...
const accountLink = await stripeClient.accountLinks.create({
   ...
   sendEmail(email,"Stripe Account Added Successfully ",accountConnectedEmail(username,email));
   sendEmail(process.env.EMAIL_USER,"A New Stripe Account " ,stripeAccountAddedAdminNotification(username,email))
    ...
    return res
      .status(500)
      .json({ message: err.message, isSuccess: false });
  }
};
...
async function createOnboardingLink(email) {
  try {
    ...
    const partnerReferralResponse = await axios.post(
      'https://api.sandbox.paypal.com/v2/customer/partner-referrals',  // Using sandbox URL for testing
      {
        ...
        partner_config_override: {
          ...
          return_url_description: 'URL to redirect after the onboarding process is complete',
    ...
    if (partnerReferralResponse.status==201) {
      ...
      const onboardingLink = partnerReferralData.links.find(link => link.rel === 'action_url').href;
...
export const getPaypaldetails = async(req,res)=>{
  const{_id} = req.user;
   try{
     ...
     if(userdata){
      return res.status(200).json({message:"Successfully fetched",data:userdata,isSuccess:true})
     }
     else{
return res.status(200).json({message:"No data found",data:{},isSuccess:false})
     }
  }catch(err){
...
export const refunds = async (req, res) => {
  ...
  if(updateStatus){
   ...
   let refund;
     ...
     if(!paymentinfo){
      return res.status(404).json({message:"Invalid Request",isSuccess:false})
     }
    if(paymentType==="paypal"){
      ...
      if (!captureId) {
        return res.status(400).json({ error: 'Capture ID is required' ,isSuccess:false});
      }
      ...
      if (response.statusCode !== 201) {
        return res.status(response.statusCode).json({ error: 'Failed to process PayPal refund' ,isSuccess:false});
      }
      ...
      return res.status(200).json({message:"Successfully refunded",isSuccess:true})
    }
  } catch (err) {
    console.log(err.message);
    return res.status(404).json({ message: "You have already Refunded " ,isSuccess:false});
  }
};
...
export const refundForuser = async (req, res) => {
  ...
  try {
    ...
    const paymentInfo = await Payment.findOne({ bookingId: bookingId, status: "Succeeded" });
    ...
    let refundAmount, avatarAmount = 0, adminAmount;
    ...
    if (paymentInfo.paymentType === "paypal") {
      ...
      if (response.statusCode !== 201) {
        return res.status(response.statusCode).json({ error: 'Failed to process PayPal refund' });
      }

      refund = response.result;

      let findrefund = await Refund.findOne({ paymentIntentId: paymentInfo.paymentIntentId });
      if (findrefund) {
    ...
    return res.status(200).json({
      ...
      refund, isSuccess: true
    });
  } catch (err) {
    console.log(err.message);
    return res.status(500).json({ message: "You have already Refunded", isSuccess: false });
  }
};
...
export const payout = async (req, res) => {
  ...
  try {
    ...
    if (findid) {
      ...
      let existingAcc = await Account.findOne({ to: to });
      ...
      if (existingAcc) {
        ...
        const tourEarning = existingAcc.TourPrice
        .reduce((acc, curr) => acc + parseFloat(curr), 0);

      // If there is another earning, add it to the total
      const additionalEarning = existingAcc.totalEarning ? parseFloat(existingAcc.totalEarning) : 0;
      ...
      let updatebooking = await Booking.findOne({ _id: updateComplete.bookingId });
...
export const withdrawInstant = async (req, res) => {
  ...
  try {
    ...
    sendEmail(StripeEmail,"Successfully Withdraw",avatarWithdrawNotification(avatarname,amount,formattedDate) )
  } catch (err) {
    console.error("Error creating instant payout:", err);
    res.status(500).json({ error: "StripeInvalidRequestError: Your destination account needs to have at least one of the following capabilities enabled: transfers, crypto_transfers, legacy_payments." });
  }
};
...
export const allstripedetails = async(req,res)=>{
  ...
  try{
    ...
    if(findid && details){
      ...
      return res.status(200).json({message:"successfully fetched",data:details,isSuccess:true});
    }
    else{
      return res.status(200).json({message:"Not found",data:{},isSuccess:false})
    }
...
export const avtartip = async (req, res) => {
  try {
    ...
    if (isNaN(tips)) {
      return res.status(400).json({ message: "Invalid tip value", isSuccess: false });
    }
  ...
  try {
    ...
    paypal.payment.execute(paymentId, express_checkout_json, async function (error, payment){
      if (error) {
        ...
        const response = JSON.stringify(payment);
        const parseresponse = JSON.parse(response);
        const updatepayment = await BookingAddon.findOne({SessionId: paymentId });
        if(updatepayment){
       ...
       if(updatebooking){
        ...
        // Redirect to the success page
        return res.redirect(`${process.env.WEBSITE_URL}/user/addtimepaymentsuccess`);
      }
    });
  } catch (err) {
    console.log("Error in paymentsuccess function:", err);
...
export const publicJoin = async (req, res) => {
  try {
    const {product,productId, avatarId, userId,bookingId, price, adminFee, paymentType } = req.body;
    const { _id } = req.user;
    const findbooking = await Booking.findOne({ _id: bookingId });
   ...
   //avatar commission
   let findOut = await userProfile.findOne({ userId: avatarId, role: "avatar" });
    let avatarcommision = findOut.avatarcommission;
    let commissionprice = (avatarcommision / 100) * price;
    const totalprice = price - commissionprice;
...
export const publicjoinsuccess = async (req, res) => {
  try {
    ...
    // Find the payment using the payment ID
    const findpayment = await PublicJoin.findOne({ paymentIntentId: paymentId });
    if (!findpayment) {
      return res.status(404).send("Payment not found.");
    }
 ...
 const captureId = payment.transactions[0].related_resources[0].sale.id;
      ...
      if (error) {
        ...
        // Update the payment status in the database
        let updateaccount = await PublicJoin.findOne({ paymentIntentId: paymentId });
        if (updateaccount) {
          updateaccount.status ="Succeeded";
          updateaccount.payerId = payerId;
          updateaccount.captureId=captureId;
        ...
        // Redirect to the success page
        return res.redirect(`${process.env.WEBSITE_URL}/user/Payment_Success_Join_Room`);
      }
    });
  } catch (err) {
    console.log("Error in paymentsuccess function:", err);
...
export const offercheckout = async (req, res) => {
  try {
    const {  product, avatarId,OfferId, price, Adminfee, paymentType } = req.body;
